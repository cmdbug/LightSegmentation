...
class Trainer(object):
    def __init__(self, args, summary=None):
        ...
        if args.resume:
            if os.path.isfile(args.resume):
                ...
                checkpoint = torch.load(args.resume, map_location=lambda storge, loc: storge)
                # 修改网络后保留原来的权重
        ...
        if self.summary:
            img = torch.rand(1, 3, args.crop_size, args.crop_size)
            self.summary.add_graph(self.model, img, verbose=False)
            self.summary.flush()
    ...
    def train(self):
        ...
        self.model.train()
        with tqdm(total=len(self.train_loader), desc=f'Train', unit='it', ncols=None) as pbar:
            for iteration, (images, targets) in enumerate(self.train_loader):
                ...
                pbar.set_postfix(**{'Step': iteration,
                                    'Iteration loss': losses.item(),
                                    'lr': self.lr_scheduler.get_last_lr()[0]})
                pbar.update()

                if self.summary and iteration % val_per_iters == 0:
                    self.summary.add_scalar('train/1_loss', losses.item(), global_step=iteration // val_per_iters)
                    self.summary.add_scalar('train/2_lr', self.lr_scheduler.get_last_lr()[0],
                                            global_step=iteration // val_per_iters)
                    self.summary.flush()
            pbar.close()
    ...
    def validation(self, iteration):
        ...
        logger.info("Sample: {:d}, Validation pixAcc: {:.3f}, mIoU: {:.3f}".format(iteration, pixAcc, mIoU))
        new_pred = (pixAcc + mIoU) / 2
        if new_pred > self.best_pred:
            is_best = True
            self.best_pred = new_pred
        if self.summary:
            self.summary.add_scalar('val/1_miou', new_pred,
                                    global_step=iteration // (self.args.val_epoch * self.args.iters_per_epoch))
...
if __name__ == '__main__':
    ...
    for f in glob.glob('../runs/logs/*events.out.tfevents*'):
        os.remove(f)
    summary = SummaryWriter(r'../runs/logs/')

    trainer = Trainer(args, summary)
    trainer.train()
    torch.cuda.empty_cache()
    summary.close()
